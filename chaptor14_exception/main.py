...
'''
from chapter01.bmi_calc.main import height
...
#     height = round(height)                  # str을 round처리 할 수가 없기 때문에 예외 발생
#     print(f"입력하신 키는 {height}cm로 처리됩니다.")
# except:
#     print("예외가 발생하였습니다..")
'''
        2) 특정 예외만 처리하는 방식
            try / except 문을 사용하면 기본적으로 예외의 종류에 상관 없이 모든 예외가 처리됨.
            하지만 이상에서 본 것처럼 0으로 나누는 경우와. 정수가 아닌 값을 입력한 경우에
            서로 다른 메시지를 출력한다면, 개발자에게 예외를 처리할 수 있을만한 정보를
            제공할 수 있음(아까 전까지의 예시는 예외 발생 수에 개발자가 어떤 코드에 문제가 있었든지
            스스로 확인해야했다는 점에서 편의 개선 측면으로 볼 수 있음.)
...
...
    ...
    예외메시지n                  # 맨마지막에는 위에 except들이 처리하지 못하는 
...
'''
5. else / finally 문
    try / except 문에 추가로 else / finally문을 사용할 수 있음.
...
6. 강제로 예외 발생시키기
    ...
    컴퓨터 상으로는 (그리고 파이썬 상으로는) -1000이 정수이기 때문에 예외가 발생하지 않스비낟.
    하지만 -1000살이 될 수 없기 때문에 직접 예외를 발생 시켜서 처리해야만 합니다.
    
    -> raise문

형식:
rasie 예외클래스(0               # 이건 사용자가 정의하는 겁니다

또는 
rasie 예외 클래스(예외메시지)

raise는 강제로 예외를 발생 시킨다는 점에서 주로 사용되는 예외 클래스는 Exception입니다.
'''
# age = int(input("나이를 입력하세요  >>>> "))       # -1000 살 입력했을 때 예외가 발생하지 않음
# print(f"당신의 나이는 {age}살입니다")                 # 파이썬 상에서는 문제가 없지만 현실 세계에서는 생기는 예외
#
# try :
#     age = int(input("나이를 입력하세요  >>>> "))
#     raise Exception("강제로 발생시킨 예외입니다.")         # 이경우 멀쩡한 숫자를 입력해도 예외가 발생합니다.
...
#     """상요자 정의 예외 클래스 : 나이가 음수일 때 발생"""
#     pass    # 예외를 발생시키기만 하면 되기 때문에 따로 코드 작성할 필요 없음
#             # -> Exception 클래스를 상속 받았기 때문에 매서드 사용 가능
#
...
#     if age > 200:
#         raise NegativeAgeError("불가능한 나이 입력입니다..")
...
ScoreOutOfRangerError 클래스를 정의해서 사용하곘습니다.
...
'''
# class ScoreOutOfRangerError(Exception):
...
#         raise ScoreOutOfRangerError("점수는 0이상 100이하입니다.")
# except ScoreOutOfRangerError as e:
#     print(e)
# except ValueError as e :
...
#     print("프로그램이 종료되었습니다.")
'''
사용자로 부터 숫자를 입력 받아 해당 숫자로 100을 나누는 값을 출력하는 프로그램을 작성하시오.
만약 사용자가 0을 입력하거나 숫자가 아닌 값을 입력하면 적절한 에외 메시지를 출력하시오.

지시 사항
1. 사용자로부터 숫자를 입력 받는다.
2. 입력 받은 숫자로 100을 나누어 결과를 출력한다.
3. 입력 값이 0일 경우 적절한 예외를 처리하여 '0으로 나룰 수 없습니다'라는 메시지를 출력한다.
