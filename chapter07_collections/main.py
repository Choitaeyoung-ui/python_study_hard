...
#     print(words.title(), end="")    # 튜플의 정의를 생각하셔야 하는데, 왜 결과값이
...
사용자로부터 임의의 양의 정수를 입력받고 그 정수 만큼 숫자를 입력 받아 list에 저장하세요
...
# li_original = input("몇 개의 숫자를 입력할까요? >>> ")
#
# for i in range(li_original):
#     li_even = input(f"{li_original}번째 숫자를 입력하세요 >>>")
#
...
# 그 요소에 해당하는 숫자가 홀수라면 두 번째 리스트에 저장되면 x 짝수라면 저장돼야함. -> 얘는 조건문

# index_num = int(input("몇 개의 숫자를 입력할까요? >>> "))
#
# for i in range(index_num):
#     num = int(input(f"{i+1}번째 숫잘르 입력하세요? >>> "))
#     li_original.append(num)

# print(f"입력 받은 숫자는 {li_original}입니다.")

# 예를 들어 첫 번째 문제만 해결하고 짝수들만 뽑아내는 조건문을 작성하시겠다면
# li_original을 반복문 돌려서 요소들만 추출하고, 그요소들이 홀/짝수 중 어디에 해당되는지를
...
#     li_original2.append(num2)   #일단 입력 받은 건 li2에 전부 추가
#     if num2 % 2 == 0:
...
사용자로부터 3 명의 이름과 전화번호를 입력받아 딕셔너리에 저장한 뒤, 입력한 정보를 추출하는
프로그램을 작성하시오.
...
#
# for i in range(3):
#     dict_name = int(input(f"{i+1}번째 사람의 입력을 입력하세요 >>> "))
#     dict_phone = int(input(f"{i+1}번째 사람의 연락처를 입력하세요 >>> "))

# telephones = {}         # 비어있는 딕셔너리를 선언하는 방법
# for i in range(3):
#     dict_key = int(input(f"{i+1}번째 사람의 입력을 입력하세요 >>> "))
#     dict_value = int(input(f"{i+1}번째 사람의 연락처를 입력하세요 >>> "))
#
...
# 이상의 코드는 가능하지만 -> add_numebers2(n)은 return으로 list형태를 반환하기 때문에
# element에 대한 추가 연산 가능 -> ex) +1을 하건, 혹은 짝수만 걸러내긴 etc...

# for number in add_numbers2(last_num):
#     print(number+1)

# 이상의 코드는 불가능합니다. -> add_numebers(n)의 경우 print 실행문으로 마무리되기 때문에 추가 연산 x
