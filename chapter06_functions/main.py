...
# my_money를 기준으로 음료수를 살 수 있는 경우의 수를 고려하면 0 ~ 4개까지 반복문이 5번 돌아가네요.
#for i in range(int(my_money//drink_pirce) + 1):      # range() 함수 내에 들어가있는 argument의 자료형의 int여야 합니다. -> 단순히 나눠서는 안되죠
#    print(f"음료수 = {i}개, 잔돈 = {my_money-(drink_pirce*i)}")
...
ctrl + shift + f 를 사용해서 구구단 출력하는 코드 찾아와서 (혹은 직접 작성하셔도 무방), 함수화 시켜보세요.
'''
# dan = 2
# while dan < 10:
#     number = 1
...
#         print(f"{dan} x {1} = {dan * i}")

dan = int(input("몇 단을 입력하시겠습니까? >>> "))

# 함수 정의-> call1 유형으로 작성했습니다.
def multiply2():
    dan = int(input("몇 단을 입력하시겠습니까? >>> "))
    for i in range(1, 10, 1):
        print(f"{dan} x {1} = {dan * i}")

# print(dan)         #현재 상황에서 오류발생 -> 함수를 정의만 하는 것은 사용한 게 아니기 때문에 변수에 해당하지 않음
# dan = 3            # 그리고 structure를 확인한 결과 선언한 dan이 없을 확인할 수 있음.
# print(dan)
