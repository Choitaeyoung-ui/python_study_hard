...
# def increaese_enemies():  # 함수 정의 시작
#    enemies = 2           # 같은 이름이기 하지만 얘는 지역 변수에 해당
#     print(f"함수 내부의 적의 숫자는 {enemies}입니다.")

# increaese_enemies()       # 함수 호출
# print(f"함수 외부의 적의 숫자는 {enemies}입니다.")

# 지역 변수 =/=  전역 변수 ->  그렇다보니깐 변수명을 서로 다르게 짓는게 혼란을 피하는 방식
...
# 지역변수가 선언되고, 이를 호출한다고 해서, 함수 내에 있는 변수가 전역에서(main에서)
#참조하는 것은 불가능합니다,
...
#def game():
#    # 함수 내부에 함수 정의 -> jump 내에서 turn_right 함수 사용했었습니다.
#     # 얘는 정의된 함수를 함수 내에서 호출 했는데, 여기서는 함수 내에서 정의를 새로 함.
...
#    drink_potion()
# game()
# print(f"체력은 {player_health}입니다.")    # 전역 변수 값을 확인
...
2. 함수 정의 내부의 if절에서 new_enemy라는 변수를 선언 및 초기화 했음에도,
    if절 바깥에서(들여쓰기 참조) new_enemy를 참조했음에도 오류가 발생하지 않음.
    
    1.의 이유 : game_level이라는 전역변수의 값을 바꾸는 게 아니라, 참조만 해서 true/false만 변환하기 때문에
        오류 발생하지 않음.
