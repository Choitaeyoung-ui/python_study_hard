...
만들어야할 변수의 개수가 많고, 학생 한 명당 매개변수가 6개라면, 학생이 100명일 경우 600개의
변수를 처리할 필요가 있습니다.
'''
...
1. 클래스의 기본 구성
    ...
    객체를 생성하기 위해서는 클래스는 객체가 가져야할 '값'과 '기능'을 지녀야합니다.
    ...
    인스턴스 매서드 : 인스턴스 변수를 사용하는 매서드
        ...
        인스턴스 매서드는 첫 번째 매개변수로 self를 추가해야합니다.
'''
# 클래스 정의
class Person:
    # chapter06_functions부분 참조하는데 -> call2()
    def set_info(self, name, age, tel, address):        # 클래스 내부에 def를 사용하면 method를 정의함
        self.name = name
        self.age = age
        self.tel = tel
    ...
    #     print(f"연락처는 : {self.name}인데, {self.age}에 살고 있습니다.")

    # call3() / 매개 변수 없음 return 있음
    def display_info2(self):
        return f"제 이름은 : {self.name}이고, {self.age} 살입니다.\n연락처는 : {self.name}인데, {self.age}에 살고 있습니다"

# 객체 생성
person01 = Person()
print(person01)     # 객체명으로 그대로 출력할 수 없음 -> 주소값만 출력
person01.set_info("최태영", 23, "010-4728-3901", "창원시 마산합포구")
...
person02.set_info("최태영", 23, "010-4728-3901", "창원시 마산합포구")
...
book2.set_info(author="어린왕자", title="생택쥐페리",stock=10)        # keyword argument
book1.display_info()
book2.display_info()
...
print(book1.title)
# 그러면 특정 속성값만 꺼내는 이유
print(book1.stock + 2)      # 연산이 가능해지기 때문
