...
while is_on:
    ...
    # 그 다음부터는 음료 이름을 정확하게 적었을 때, 혹은 오타났을 때에 해당합니다.
        ...
        if drink == None:   # choice에 오타가 있을 경우 None이 반환되기 때문에 작성
            continue        # contiune가 실행되면 이 밑의 코드라인은 실행되지 않고,
        ...
        if coffee_maker.is_resource_sufficient(drink):
                # coffee_maker.is_resource_sufficient() 매서드를 확인해보면,
    # drink.ingredients를 반복문 돌린다는 것을 확인할 수 있습니다.
    # 이제 여러분들이 주의해서 봐야할 점은 미리 작성된 매서드가 어떤 자료형의 argument를
    # 요구하고 있는지입니다.
    # 즉, pop version에서처럼 동일하게 생각해서 drink.ingredients를 argument로 넣었다면
