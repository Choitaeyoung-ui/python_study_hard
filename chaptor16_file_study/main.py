...
from chapter07_hangman.hangman_complete.hangman06 import end_of_game
...
속성을 추가
read() 매서드를 통해 전체를 읽으려면 메모리 공간이 많이 필요합니다. 읽어야 할 파일이 크다면
...
'''
        3) readlines() 매서드
            전체 라인을 읽어들여서 각 라인 단위로(개행 단위로) '리스트'에 저장하는 매서드
'''
...
'''
나라별 수도를 순차적으로 반복 시켜 nation 리스트에 사전에 미리 저장해두었습니다.

nation 리스트에 내용을 이해하여 다음과 같은 nation.txt 파일을 '생성'하세요.
