...
정해야하는 경우에 적합함. 정적 메서드는 클래스에 소속돼있지만 인스턴스에는 영향을 주지도 않고,
또한 영향을 받지도 않음.
'''
# class Korean3:
#     country = "한국"      # 클래스 변수
...
#
#     def __int__(self):      # 생성자 / 인스턴스 매서드에 해당함 -> 아무 @가 없으니까요
#         Bag.count += 1      # 객체를 하나 생성할 때마다 클래스 변수인 count가 1씩 증가함.
#         print("가방이 생상되었습니다.")   # 인스턴스 매서드가 클래스 변수를 참조할 때는
#                                         # 클래스명.클래스변수명으로 접근함.
#
#     # 클래스 매서드 정의
#     @classmethod
...
#         return cls.count    # 클래스 매서드를 생성(getter 형태로)하여 참조하는 방식
#
# print(f"현재 가방 재고 : {Bag.count}")        # 클래스 변수를 직접 참조
# print(f"현재 가방 재고 : {Bag.remain_bag()}") # 클래스 변수를 getter를 통해 매서드를 경유하여 참조 -> 보안상 더 좋습니다.
#
# # 객체 생성
...
# bag1.sell()     # 인스턴스명.클래스매서드명()으로 호출 -> 가능했었죠 // 이 경우는 사실 좀 애매한게
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
# Bag.sell()      # 클래스명.클래스매서드명()으로 호출 -> 더 권장되는 방식
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
'''
이상의 코드에서 보면 예시이기 때문에 적합하다고 보기가 힘든데,
인스턴스명.sell()로 작성한다면 특정하는 가방 인스턴스가 팔려다고 볼 수 있기 때문에 가독성이 더 높은 반명
...
print(f"전체 인구수 : {Person.get.population()}")
...
1. Shop 클래스는 다음과 같은 변수를 가지고 있다.
    total : 가게 전체 매출액
    menu_list : {메뉴명:가격}으로 이루어진 딕셔너리 요솔르 가지는 리스트
...
class Shop:
    ...
    def sales(cls, menu_name, quantity):
        # 생각해보셔야 할게 menu_name을 argument로 받았을 때, 걔가 리스트 내의 요소의 key에 해당하는지
        ...
        # 이상의 부분은 collection의 요소들을 가져오는 반복문에 해당되는데
        # argument로 menu_name을 받은 뒤에 그게 일치하는 key를 찾아내고, 그것은 value * quantity해준 값을
        # total에 더해주면 되지 않을까요

Shop.sales2("떡볶이", 1)    # 떡볶이을(를) 1개 판매
Shop.sales2("김밥", 2)     # 김밥을(를) 2개 판매
Shop.sales2("순대", 5)     # 순대을(를) 5개 판매
