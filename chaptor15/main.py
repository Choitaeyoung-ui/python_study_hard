...
'''
1. 상속이란?
    어떤 클래스가 가지고 있는 기능을 그대로 물려받아서 사용할 수 있는 클래스를 생성할 수 있는 데,
    클래스의 긴을을 물려 받은 게 상속 받았는다는 표현을 사용함.
    상속관게에 있는 클래스를 부모(슈퍼) 클래스 / 자식(서브) 클래스라는 용어를 사용함.

2. 상속관계 구현
    두 클래스가 상속 관계에 놓이려면 IS-A 관계를 성립해야함. IS-A 관계란 "~은 ~이다"로 해석할 수 있는
    관께를 의미
        ex) 학생은 사람이다.

    *IS-A 원문 : is a kind od -> Dog is a kind of Animal -> '개'는 '동물'의 일종이다
...
potter.eat("감자")  # Student 클래스에는 없는 부모 클래스의 매서드인 eat()을 호출 가능
potter.study()  # 자식 클래싀 매서드 호출

'''
3. 서브 클래스의 __int__()

서브 클래스는 슈퍼 클래스가 없으면 존재할 수 없습니다. 그러나 서브 클래스의 생성자를 구현할 때는
'반드시 슈퍼 클래스의 생성자를 먼저 호출'하는 코드를 작성해야함ㄴ 합니다.

super -> 슈퍼 클래스를 의미. 즉 Student의 생성자를 호출하려면 super().__int__(name)에 의해서
슈퍼 클래스인 Person의 생성자가 먼저 호출되면서 슈퍼 클래스의 객체가 생성됩니다.
이후 슈퍼 클래스에서 생성된 인스텉스 변수인 name이 서브 클래스로 전달되고 이후에 서브 클래스에서 school
인스턴스를 선언 및 초기화하여 저장하면서 서브 클래스의 객체가 생성됩니다.

4. 서브 클래스의 인스터스 자료형

슈퍼 클래스의 객체는 슈퍼 클래스의 인스턴스
하지만 서브 클래스의 객체는 서브 클랫의 인스턴스이며냐서 슈퍼 클래스의 인스턴스
Stuent 클래스의 객체는 Stduent의 인스턴스이면서 동시에 Person의 인스턴스라는 의미입니다.
...
2. 서브 클래스 Hybrid는 다음과 같은 특징을 가지고 있습니다.
    ...
    현재 주유량과 충전량을 모두 확인할 수 있는 hybrid_info() 매서드가 있습니다.
...
class Car:
    ...
    def __int__(self, oil):     # 생성자
        self.oil = oil
    def add_oil(self, oil):     # 매서드
        if oil <= 0:
            return              # 매서드 정의 후에 return 후 아무 값도 쓰지 않으면 매서드 종류를 의미
        self.oil += oil
