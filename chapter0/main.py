...
1. = : = 오른쪽에 있는 데이터를 = 왼쪽에 있는 변수에 대입한다는 의미.
    즉, 94번 라인의 b = 1는 1이라는 정수 데이터를 b라는 변수에 대입하고,
    96번 라인에서 b(1) + 1라는 데이터를 다시 b라는 변수에 대입했다는 의미로
    그 결과 97번 라인에서 2가 출력됨.
...
138-140라인까지의 코드 분석합니다. input() 함수는 () 안에 있는 질문 사항(프롬프트)을
콘솔에 출력하여 사용자로 하여금 입력받을 수 있게끔 합니다.
사용자가 프롬프트에 맞는 정보를 입력하면, 그 데이터는 name이라는 변수에 저장되고,
이를 print() 함수를 통에 콘솔로 출력하게 됩니다.
